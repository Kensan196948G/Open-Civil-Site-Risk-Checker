...
| 画面                     | 内容                                                                                                                                                                                                                                                                                                                                |
| ------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| SCR-000 ダッシュボード   | 既定の起動画面。調査案件一覧・KPI・確認優先度の全体集計。**案件一覧の検索・絞り込み**（キーワード: 案件名/コード/所在地・状態フィルタ・件数表示・クリア）。**本番データ（実取得）案件**と**ダミー（サンプル）案件**を区別表示。JSON 取込/エクスポート対応。**案件台帳（サーバー保存）**セクション（Issue #111・API 有効時のみ表示）：一覧・**「開く」で保存済み確認結果を復元表示**・承認申請・承認・監査履歴表示・承認者表示 + **状態サマリー（draft / 承認待ち / approved の件数）**                                                                                                                                                                             |
