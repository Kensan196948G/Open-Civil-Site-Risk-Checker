...
| 画面                     | 内容                                                                                                                                                                                                                                                                                                                                |
| ------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| SCR-000 ダッシュボード   | 既定の起動画面。調査案件一覧・KPI・確認優先度の全体集計。**案件一覧の検索・絞り込み**（キーワード: 案件名/コード/所在地・状態フィルタ・件数表示・クリア）+ **CSV エクスポート（絞り込み結果・UTF-8 BOM 付き）**。**本番データ（実取得）案件**と**ダミー（サンプル）案件**を区別表示。JSON 取込/エクスポート対応。**案件台帳（サーバー保存）**セクション（Issue #111・API 有効時のみ表示）：一覧・**「開く」で保存済み確認結果を復元表示**・承認申請・承認・監査履歴表示・承認者表示 + **状態サマリー（draft / 承認待ち / approved の件数）** + **案件台帳 CSV エクスポート（UTF-8 BOM 付き）**                                                                                                                                                                             |
| SCR-001 地点入力         | 住所 / 緯度経度・検索半径（100m〜3km）・確認カテゴリ選択、入力検証。**調査テンプレート（評価書 #21）**: 標準調査・道路工事・河川・護岸工事・建築・造成工事の工種別プリセットで半径・カテゴリの初期値を一括適用（適用後も個別調整可）。**最近の住所（評価書 #11）**: 分析に成功した住所入力を localStorage に最大5件保存し、ワンクリックで再入力                                                                                                                                                                                                                                                                  |
...
| SCR-010 候補地比較       | 保存済み案件（実データ + **サーバー台帳案件（#111・有効時）** + ダミー）から 2〜4 地点を選択し、主要リスク要素（災害・河川・地形・気象・道路・施設）を横並び比較。**データ未取得と低リスクを区別**・断定表現なし。**候補地の位置関係マップ**（選択順の番号つきマーカー + 検索範囲円・fitBounds で全体表示）。**地図キャプチャ（#274 方式）**: 表示中のマップを画像化し A4 印刷 / PDF の「候補地の位置関係」セクションへ同梱（出典・取得日時を明示）。**気象警報（現在）チェック（評価書 #18）**: 各候補地の都道府県で現在発表中の警報・注意報を気象庁から取得し表示（発表単位の注記つき）。チェック結果は A4 印刷 / PDF の「気象警報（現在）」セクションに同梱Markdown / CSV エクスポート対応 + **A4 印刷 / PDF（#113 調査パックと同方式）**。選択が無い場合は架空のデモ比較を表示（Issue #175）                                                                                                                                                                                                                         |
| SCR-006 データソース管理 | 接続状態・利用条件の台帳、接続テスト（実疎通・**全ソースの一括テスト**）。**データ鮮度・ライセンス台帳（Issue #174）**: 行クリックで元データ更新日・利用条件メモ・再取込履歴を表示。レポートの出典セクションへ鮮度・利用条件を自動埋め込み                                                                                                                                                                                                                                                                                      |
| SCR-007 取得ログ         | 実行履歴（成功 / 失敗 / タイムアウト / スキップを区別）。**フィルタ・検索**: ソース / 状態 / エンドポイント / キーワード（エラー・コード）で AND 絞り込み・件数表示（未実施・目視のみも選択可能）+ **CSV エクスポート（絞り込み結果・UTF-8 BOM 付き）**                                                                                                                                                                                                                                                                             |
| SCR-009 監査ログ         | 案件台帳の操作履歴（Issue #111・auditor ロール）。actor・時刻・対象・action を表示（本文・秘密情報は含まない）。API 未設定時は架空のダミー監査ログを表示。**CSV エクスポート（監査証跡・ISO/J-SOX・UTF-8 BOM 付き）** + **フィルタ・検索（actor / action / 対象 / キーワード / 日付レンジ（監査期間）・AND 結合・件数表示）**                                                                                                                                                                                                                                  |
...
### 🧭 その他の新機能のデモ・確認手順（調査テンプレート / 比較マップキャプチャ / AI 利用状況）

```text
A) 調査テンプレート（SCR-001・評価書 #21）
   1) 地点入力画面で「調査テンプレート（初期値の一括適用）」から工種を選択
      （標準調査・道路工事・河川・護岸工事・建築・造成工事）
   2) 検索半径・確認カテゴリがテンプレートの初期値に変わることを確認（個別調整も可能）

B) 候補地比較の地図キャプチャ（SCR-010・#274 方式）
   1) 比較画面で 2〜4 地点を選択 → 「候補地の位置関係」にマップ表示
   2) 「📷 地図画像を取得」→ 「🖨 印刷 / PDF」で「候補地の位置関係（地図キャプチャ）」
      セクションに地図画像・出典・取得日時が同梱されることを確認

C) AI 利用状況（SCR-008・評価書 #20）
   1) システム設定の AI 設定セクションに「AI 利用状況（直近30日・サーバー記録）」を表示
   2) 呼び出し数・成功/失敗・文字数・概算費用・ユーザー別・日別が表示されることを確認
      （サーバー側 DB ai_usage に記録された実績のみ・プロンプト本文は記録されません）

D) 気象警報（現在）チェック（SCR-010・評価書 #18）
   1) 比較画面で 2〜4 地点を選択 → 「⚠ 気象警報（現在）をチェック」
   2) 候補地別に「発表中（警報名）/発表なし/取得失敗」のバッジが表示されることを確認
      （都道府県の気象庁発表単位・地点そのものが対象地域と一致するとは限りません）
```
