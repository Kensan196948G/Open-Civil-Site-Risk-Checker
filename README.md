...
| `backend/`（ocsrc-api） | KSJ 空間検索 API（読み取り専用 3 エンドポイント） | 127.0.0.1 バインド。KSJ 連携は SCR-008 で URL 設定時のみ有効 |
...
- 🔌 バックエンド接続先は2通りで設定できます: ①ビルド時 `VITE_OCSRC_BACKEND_URL`（例 `http://127.0.0.1:8000`）、②**SCR-008 システム設定画面からランタイムで設定**（ビルド不要・保存後すぐ反映、①より優先）。**どちらも未設定なら従来どおり「未連携」表示**で動作は不変です。本番構成では SPA 配信オリジン（`:8700`）を設定すると `/api/*` same-origin プロキシ経由で 127.0.0.1 バインドの API へ到達します
