...
Memory は user scope と project scope を分離して扱うこと。

- user scope: 公式 Memory MCP。全プロジェクト共通のユーザー方針・好みだけを参照する。
- project scope: ByteRover/Cipher 系。リポジトリ固有の設計判断・修正理由・失敗パターンを参照する。
- project 補助: Serena。symbol 単位の調査・編集が必要な場合だけ使う。

競合時は `現在のユーザー指示 > AGENTS.md/CLAUDE.md/repo docs > project memory > user memory > モデル推測` の順で判断する。
token / secret / credential / CI ログ全文 / ブランチ一時情報は memory に保存しない。
